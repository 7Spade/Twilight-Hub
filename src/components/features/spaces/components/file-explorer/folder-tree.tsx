...
export function FolderTree({ files, selectedItems, onSelectionChange, onItemClick, onItemAction }: FolderTreeProps) {
  ...
  // Â∞áÁ?ÂØ¶Ê?Ê°àÊï∏?öÁ?ÁπîÊ?Ë≥áÊ?Â§æÁ?Êß?
  const organizeFilesIntoFolders = (fileList: FileItem[]) => {
    const folders: { [key: string]: FileItem[] } = {};
    
    // ?πÊ?Ê™îÊ?È°ûÂ??ñÂ?Á®±Â?Á∂¥Á?ÁπîÊ?Ê°?
    fileList.forEach(file => {
      let folderName = '?∂‰?Ê™îÊ?';
      
      // ?πÊ?Ê™îÊ??çÁ®±?çÁ∂¥?§Êñ∑Ë≥áÊ?Â§?
      if (file.name.startsWith('A000') || file.name.startsWith('A100')) {
        ...
        folderName = 'CADÊ™îÊ?';
      } else if (file.name.includes('Contract') || file.name.includes('?àÁ?')) {
        folderName = '?àÁ??á‰ª∂';
      } else if (file.name.includes('Report') || file.name.includes('?±Â?')) {
        folderName = '?±Â?';
      }
  ...
  // ‰ΩøÁî®?üÂØ¶Ê™îÊ??∏Ê?ÁµÑÁ??ÑË??ôÂ§æÁµêÊ?
  const organizedFolders = organizeFilesIntoFolders(files);

  // Ê®°Êì¨Ë≥áÊ?Â§æÊï∏?öÔ??®ÊñºÊ∏¨Ë©¶?∂Â??üËÉΩ - ?¥Ë?ÂØåÁ?Â±§Á?ÁµêÊ?
  ...
  // ?à‰ΩµÊ®°Êì¨Ë≥áÊ?Â§æÂ??üÂØ¶Ê™îÊ?Ë≥áÊ?Â§?
    ...
    <div className="h-full flex flex-col">
      <Tabs defaultValue="folders" className="w-full">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="folders">Ë≥áÊ?Â§?/TabsTrigger>
          <TabsTrigger value="packages">Â•ó‰ª∂</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="folders" className="mt-2">
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-2 py-1.5 px-2 hover:bg-gray-100 cursor-pointer group rounded-sm bg-gray-50">
              ...
              <span className="text-sm font-medium text-gray-700">Â∞àÊ?Ê™îÊ?</span>
              <ContextMenu
                item={{ id: 'project-files', name: 'Â∞àÊ?Ê™îÊ?', type: 'folder', timeCreated: '', updated: '' }}
                onAction={(action) => onItemAction?.({ id: 'project-files', name: 'Â∞àÊ?Ê™îÊ?', type: 'folder', timeCreated: '', updated: '' }, action)}
              >
                <button
                  className="h-6 w-6 p-0 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-gray-200 rounded-sm flex items-center justify-center ml-auto"
                  onClick={(e) => e.stopPropagation()}
