...
const ExtractEngagementDataInputSchema = z.object({
  fileDataUri: z
    .string()
    .describe(
      "A document (contract, quote, etc.) as a data URI. Format: 'data:<mimetype>;base64,<encoded_data>'."
    ),
});
...
const ExtractEngagementDataOutputSchema = z.object({
  ...
  totalValue: z
    .number()
    .describe('The total value before tax (subtotal).'),
...
export type ExtractEngagementDataOutput = z.infer<
  typeof ExtractEngagementDataOutputSchema
>;
...
export async function extractEngagementData(
  input: z.infer<typeof ExtractEngagementDataInputSchema>
